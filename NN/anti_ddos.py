...
def add_samples_to_training_set(training_set, file_name, label):
    with open(file_name) as file_:
        for line in file_:
            try:
                entry = LogEntry(*log_re.match(line).groups())
...
if __name__ == '__main__':
    ...
    parser.add_option("-g", "--good", dest="good_file",
                              help="log with good traffic before DDoS", metavar="FILE")
    parser.add_option("-b", "--bad", dest="bad_file",
                              help="log with bots' requests.", metavar="FILE")
    parser.add_option("-l", "--log", dest="log_file",
                              help="log for classification.", metavar="FILE")
    (options, args) = parser.parse_args()

    log_re = re.compile(r'(?P<ip>[0-9.:a-f]+) [^ ]+ [^ ]+ \[.*\] "(?P<url>.*)" (?P<code>[0-9]+) (?P<size>[0-9]+) "(?P<refer>.*)" "(?P<useragent>.*)"$')
        ...
        with open(options.bad_file) as bad_file:
            for line in chain(good_file, bad_file):
                try: 
                    ...
                    log.error('Failed to parse line: {0}'.format(line), exc_info=True)
    log.info('Feature vector size: {0}'.format(len(dictionary)))
    ...
    for _ in xrange(tries):
        log.info('Constructing NeuralNetwork...')
    ...
    log.info('Activating NeuralNetwork...')
    ...
    with open(options.log_file) as log_file:
        ...
        for line in log_file:
            try:
                entry = LogEntry(*log_re.match(line).groups())
                if out[cnt]:
                    f.write("BOT:  " + "{0}".format(entry) + '\n')
                else:
                    f.write("GOOD:  " + "{0}".format(entry) + '\n')
